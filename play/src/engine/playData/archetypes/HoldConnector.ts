...
import { isInLane, markAsNoEmpty } from './InputManager'
...
export class HoldConnector extends Archetype {
    ...
    preprocess() {
        ...
        this.despawnTime = Math.max(
            this.headSharedMemory.targetTime,
            this.tailSharedMemory.targetTime,
        )
    ...
    initialize() {
        ...
        this.lastActiveTime = -1000
        this.exportStartTime = -1000
    }

    updateSequential() {
        if (time.now < this.targetTime.min || time.now >= this.targetTime.max) return
        ...
        const lane = Math.remap(t.min, t.max, this.lane.min, this.lane.max, 0)

        for (const touch of touches) {
            if (!isInLane(touch.position, lane, 0.9)) continue

            this.lastActiveTime = time.now

            if (touch.started) markAsNoEmpty(touch)
        }

        if (this.lastActiveTime === time.now) {
    ...
    updateParallel() {
        ...
        if (this.lastActiveTime === time.now) {
            if (this.shouldPlaySFX && !this.sfxInstanceId)
                this.sfxInstanceId = effect.clips.loop(this.getSFX())
    ...
    get shouldScheduleSFX() {
        return options.sfxEnabled && options.autoSFX && this.headImport.holdSfx !== SFX.None
    }
    ...
    getIsActive(lane: number) {
        for (const touch of touches) {
            if (isInLane(touch.position, lane, 0.9)) return true
        }
    ...
    scheduleSFX() {
        const id = effect.clips.scheduleLoop(this.getSFX(), this.headSharedMemory.targetTime)
        effect.clips.scheduleStopLoop(id, this.tailSharedMemory.targetTime)
    }
